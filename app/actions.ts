...
export async function enrollStudent(formData) {
  try {
    ...
    if (result.acknowledged) {
      ...
      await cookies().set('lastEnrollment', result.insertedId.toString(), { 
        maxAge: 3600, // 1 hour
        path: '/',
      });
...
export async function uploadCourse(formData: FormData) {
  try {
    ...
    if (!validImageTypes.includes(thumbnail.type)) {
      return { success: false, message: 'Thumbnail must be JPEG, PNG, or WebP format' };
    }
    
    if (thumbnail.size > 2 * 1024 * 1024) {
      return { success: false, message: 'Thumbnail size must be less than 2MB' };
    }
...
export async function fetchCourses({ 
  ...
  try {
    ...
    // Fetch courses
    const courses = await collection.find(filter)
      .sort(sort)
      .skip(skip)
      .limit(limit)
      .toArray();
...
export async function downloadCoursePDF(courseTitle: string) {
  try {
    ...
    return { 
      success: true, 
      fileName: course.brochure.name || `${course.title.replace(/\s+/g, '_')}_brochure.pdf`,
